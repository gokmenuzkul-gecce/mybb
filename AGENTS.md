...
| `advanced_sponsor_manager` | Sponsor Yönetimi | sponsors, items (+ settings deep link) |
...
Advanced sponsor/ad manager. Zones render from `$GLOBALS['advanced_sponsors_top']`
/ `_bottom` (header/footer placeholders), `$GLOBALS['advanced_category_sponsor']`
(forumdisplay/showthread) and the sticky edge banners painted in `pre_output()`.

Slot contract — a sponsor row has both a `slot` and a `position`:

* `slot` = `single` | `duo` | `forum` | `edge`; `position` = `top` | `bottom` | `edge`.
* `render_category($fid)` pulls `slot='forum'` rows for that fid (or `fid=0`,
  board-wide). `advanced_sponsor_manager_fetch()` is the zone path and must
  **not** return `forum`/`edge` rows — otherwise a category sponsor pops up in
  the top zone as a duplicate. It filters `slot IN ('single','duo')` for
  non-edge positions and `slot='edge'` for the edge zone. Keep those slot
  filters when adding positions.
* `fid` is only meaningful for `slot='forum'`; `edge` only for the edge slot.

Every outbound link goes through `reklam.php?sid=N[&iid=M]` (generated by
`advanced_sponsor_manager_write_redirect_script()`), which appends the
`affiliate` tracking code server-side and counts the click. The sponsor panel
(`modcp.php?action=asm_sponsor`) never exposes `affiliate`, so a sponsor cannot
strip it. Click dedupe is per-`sid`+IP inside `asm_click_dedupe` seconds, so a
rapid double-click on the same banner counts once but a distinct product
(`iid`) on the same sponsor still needs its own dedupe window.

Panel access needs **both** core `canmodcp=1` and the plugin's
`asm_sponsor_panel` usergroup flag (admins pass via `cancp`). The core gate is
line 45 of `modcp.php`, so a user without `canmodcp` gets the generic
no-permission page before the plugin ever runs — do not test the panel with a
plain member. `advanced_sponsor_manager_panel_owned()` enforces ownership on
every write.

Templates and the ACP module are recreated idempotently on activate; the
placeholders in `header`/`footer` are inserted once (count stays 1 across
repeated activates). `uninstall()` drops the tables (data is intentionally not
preserved), removes the templates, the ACP module dir, the usergroup column and
`reklam.php`, and is safe to follow with a fresh `install()`.

Moderator thread ads: a moderator picks one of their forum's `slot='forum'`
sponsors on the new-thread form (`newthread_start` fills
`{$asm_thread_ad_selector}`), and the choice is stored in `asm_thread_ads` at
`newthread_do_newthread_end`. The save re-checks that the sponsor belongs to the
posted `fid`, so a crafted POST cannot attach another category's sponsor. The
banner renders from `showthread_end` into `{$asm_thread_ad}`, right below the
category slot and gated by the same `asm_guest_visible` rule as every zone.

Two settings drive the front end here: `asm_guest_visible` (off = zones hidden
from logged-out traffic) and `asm_show_clicks` (off = click counts only for
`cancp` staff, on = shown to everyone). Both are read through
